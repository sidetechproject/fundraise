...
<div class="popup-share popup-invite-{{$startup->id}}">
    <header>
        <span>Invite Startups</span>
        <div class="close"><i class="uil uil-times"></i></div>
    </header>
    <div class="content">
        <p>Invite founders to add their startups to <strong>{{ $startup->name }}</strong>.</p>

        <form action="{{route('send_invite_startup')}}" method="post" enctype="multipart/form-data" data-parsley-validate>
            <input type="hidden" id="invite" name="_method" value="POST">
            <input type="hidden" id="startup_id" name="startup_id" value="{{$startup->id}}">
            @csrf

            <div class="textarea">
                <textarea type="textarea" name="emails" placeholder="startup-1@email.com; startup-2@email.com" rows="10"></textarea>
        ...
        <div class="mt-5 w-100">
            Or, invite via link: {{route('invite_fund', $startup->slug)}}
        </div>
    </div>
</div>

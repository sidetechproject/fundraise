<div class="places-item hover__box">
    <div class="places-item__thumb hover__box__thumb">
        <a title="{{$place->name}}" href="{{route('place_detail', $place->id)}}"><img src="{{getImageUrl($place->thumb)}}" alt="{{$place->name}}"></a>
    </div>
    ...
    <div class="places-item__info">
        <div class="places-item__category">
            @foreach($place['place_types'] as $type)
                <a href="#" title="{{$type->name}}">{{$type->id}}</a>
            @endforeach
        </div>
        <h3><a href="{{route('place_detail', $place->slug)}}" title="{{$place->name}}">{{$place->id}}</a></h3>
